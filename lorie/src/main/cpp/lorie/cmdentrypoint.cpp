...
static std::atomic_bool serverStartClaimed(false);
static volatile int serverReady = 0;

extern "C" int lorieIsServerReady(void) {
    return __atomic_load_n(&serverReady, __ATOMIC_ACQUIRE);
}

extern "C" void lorieMarkServerReady(void) {
    __atomic_store_n(&serverReady, 1, __ATOMIC_RELEASE);
}

static void freeOwnedArgv(OwnedArgv *ownedArgv) {
    if (!ownedArgv)
        return;
    if (ownedArgv->pointers) {
        for (int i = 0; i < ownedArgv->argc; ++i)
            free(ownedArgv->pointers[i]);
        free(ownedArgv->pointers);
    }
    free(ownedArgv);
}

static bool isApplicationArgument(const char *arg) {
    return arg && strncmp(arg, "--desktop-owner=", 16) == 0;
}
...
Java_com_termux_x11_CmdEntryPoint_start(JNIEnv *env, __unused jclass cls, jobjectArray args) {
    ...
    if (serverStartClaimed.exchange(true)) {
        log(ERROR, "X11 server start requested more than once in this process");
        return JNI_FALSE;
    }

    // execv's argv array is a bit incompatible with Java's String[], so copy
    // every value into storage owned by the server thread.
    const jsize java_argc = env->GetArrayLength(args);
    int server_argc = 1; // Leading executable path
    for (jsize i = 0; i < java_argc; ++i) {
        auto js = (jstring) env->GetObjectArrayElement(args, i);
        if (!js) {
            log(ERROR, "Null Java argument at index %d", i);
            return JNI_FALSE;
        }
        const char *pjc = env->GetStringUTFChars(js, JNI_FALSE);
        if (!pjc) {
            env->DeleteLocalRef(js);
            log(ERROR, "GetStringUTFChars failed for argument %d", i);
            return JNI_FALSE;
        }
        if (isApplicationArgument(pjc)) {
            log(INFO, "JNI APP_ARG consumed before dix_main: --desktop-owner=<redacted>");
        } else {
            ++server_argc;
        }
    ...
    auto *ownedArgv = (OwnedArgv*) calloc(1, sizeof(OwnedArgv));
    if (!ownedArgv) {
        log(ERROR, "Unable to allocate native argv state");
        return JNI_FALSE;
    }
    ownedArgv->argc = server_argc;
    ownedArgv->pointers = (char**) calloc((size_t) server_argc + 1, sizeof(char*));
    if (!ownedArgv->pointers) {
        freeOwnedArgv(ownedArgv);
        log(ERROR, "Unable to allocate native argv");
        return JNI_FALSE;
    }
    ownedArgv->pointers[0] = strdup("Xlorie");
    int native_index = 1;
    for (jsize i = 0; i < java_argc; ++i) {
        auto js = (jstring) env->GetObjectArrayElement(args, i);
        if (!js) {
        ...
        if (!isApplicationArgument(pjc)) {
            ownedArgv->pointers[native_index++] = strdup(pjc);
        }
        env->ReleaseStringUTFChars(js, pjc);
        env->DeleteLocalRef(js);
    }
    ...
    int threadResult = pthread_create(&t, nullptr, +[](__unused void* cookie) -> void* {
        auto *ownedArgv = static_cast<OwnedArgv*>(cookie);
        if (getenv("TERMUX_X11_DEBUG")) {
            log(INFO, "Native argv before X11 option parsing: argc=%d", ownedArgv->argc);
            for (int i = 0; i < ownedArgv->argc; ++i) {
                char hex[1024] = {0};
                size_t offset = 0;
                const unsigned char *bytes =
                    (const unsigned char*) ownedArgv->pointers[i];
                for (size_t j = 0; bytes[j] && offset + 4 < sizeof(hex); ++j) {
                ...
                log(INFO, "argv[%d]=\"%s\" hex=%s", i,
                    ownedArgv->pointers[i], hex);
            }
        }
        int result = dix_main(
            ownedArgv->argc,
    ...
    if (threadResult != 0) {
        freeOwnedArgv(ownedArgv);
        log(ERROR, "Failed to create X11 server thread: %s", strerror(threadResult));
        return JNI_FALSE;
    }
    return JNI_TRUE;
}
...
Java_com_termux_x11_CmdEntryPoint_listenForConnections(JNIEnv *env, jobject thiz) {
    ...
    // Do not signal ACTION_START until the X11 screen is initialized. The
    // renderer requests its shared X connection immediately after the signal,
    // and that request requires pvfb/state to exist.
    while (!lorieIsServerReady())
        usleep(1000);
