...
present_restore_screen_pixmap(ScreenPtr screen)
{
    ...
    /*
     * Lorie calls this during a display-size transition even when no Present
     * page flip is active. There is then nothing to restore. Aborting the X
     * server here leaves the Android renderer disconnected.
     */
    if (!flip_pixmap) {
        ErrorF("present: no flip pixmap while restoring screen; skipping restore\n");
        return;
    }
