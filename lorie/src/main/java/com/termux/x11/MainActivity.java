...
public class MainActivity extends AppCompatActivity {
    ...
    private static Bundle extractConnectionBundle(Intent intent) {
        ...
        Log.d("MainActivity", "Extracting connection bundle from intent=" + intent);

        // Try canonical named key first
        Bundle bundle = intent.getBundleExtra(CmdEntryPoint.EXTRA_CONNECTION_BUNDLE);
        if (bundle != null) {
            Log.d("MainActivity", "CONNECTION_BUNDLE_PRESENT true (named key)");
            return bundle;
        }

        // Fallback to legacy null key
        bundle = intent.getBundleExtra(null);
        if (bundle != null) {
            Log.d("MainActivity", "CONNECTION_BUNDLE_PRESENT true (null key)");
            return bundle;
        }

        // Deep search in extras
        Bundle extras = intent.getExtras();
        if (extras != null) {
            Object raw = extras.get(CmdEntryPoint.EXTRA_CONNECTION_BUNDLE);
            if (raw instanceof Bundle) {
                Log.d("MainActivity", "CONNECTION_BUNDLE_PRESENT true (extras.get named)");
                return (Bundle) raw;
            }
            bundle = extras.getBundle(null);
            if (bundle != null) {
                Log.d("MainActivity", "CONNECTION_BUNDLE_PRESENT true (extras.getBundle null)");
                return bundle;
            }
        }

        Log.w("MainActivity", "CONNECTION_BUNDLE_PRESENT false");
        return null;
    }

    private static IBinder extractConnectionBinder(Bundle bundle) {
        if (bundle == null) return null;

        // Try canonical named key first
        IBinder binder = bundle.getBinder(CmdEntryPoint.EXTRA_CONNECTION_BINDER);
        if (binder != null) {
            Log.d("MainActivity", "CONNECTION_BINDER_PRESENT true (named key)");
            return binder;
        }

        // Fallback to legacy null key
        binder = bundle.getBinder(null);
        if (binder != null) {
            Log.d("MainActivity", "CONNECTION_BINDER_PRESENT true (null key)");
            return binder;
        }

        Log.w("MainActivity", "CONNECTION_BINDER_PRESENT false");
        return null;
    }
    ...
    public static void cancelPendingLaunch() {
        if (sPendingLaunchRunnable != null) {
            sLaunchHandler.removeCallbacks(sPendingLaunchRunnable);
    ...
    public static void launchOrReuse(Context context, Bundle connectionBundle, String launchToken) {
        if (connectionBundle != null) {
            sPendingConnectionBundle = connectionBundle;
            // Cache immediately so we are ready when the timer expires, but DO NOT call setDesktopReady() here.
            // Authority belongs to authoritative native connection verification in tryConnect().
            DesktopNavigationState.setLastConnectionBundle(connectionBundle);
        }
        if (launchToken != null) {
            sPendingLaunchToken = launchToken;
        }

        if (DesktopNavigationState.isDesktopLaunchPending()) {
            Log.i("MainActivity", "Launch already pending; ignoring duplicate");
            return;
        }
        ...
        sPendingLaunchRunnable = () -> {
            ...
            Bundle bundleToUse = sPendingConnectionBundle != null ? sPendingConnectionBundle : DesktopNavigationState.getLastConnectionBundle();
            String tokenToUse = sPendingLaunchToken != null ? sPendingLaunchToken : DesktopNavigationState.getPendingDesktopLaunchToken();

            if (bundleToUse != null) {
                intent.putExtra(CmdEntryPoint.EXTRA_CONNECTION_BUNDLE, bundleToUse);
                intent.putExtra(null, bundleToUse);
                if (bundleToUse.getBinder(CmdEntryPoint.EXTRA_CONNECTION_BINDER) == null
                    && bundleToUse.getBinder(null) != null) {
                    bundleToUse.putBinder(CmdEntryPoint.EXTRA_CONNECTION_BINDER,
                        bundleToUse.getBinder(null));
                }
            }
            if (tokenToUse != null) {
    ...
    private static Runnable thisRunnable() {
        return () -> {
            Runnable pending = sPendingLaunchRunnable;
    ...
    public void onBroadcastReceive(Context context, Intent intent) {
        if (intent == null) {
            Log.w("MainActivity", "ACTION_START received with null intent");
            return;
        }
        prefs.recheckStoringSecondaryDisplayPreferences();
        if (ACTION_START.equals(intent.getAction())) {
            try {
                // Ensure we don't have multiple tryConnect chains running
                handler.removeCallbacks(tryConnectRunnable);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (ACTION_START.equals(getIntent().getAction())) {
    ...
    private void initHomeOverlay() {
        final View homeOverlay = findViewById(R.id.home_overlay);
        if (homeOverlay == null) return;

        homeOverlay.setOnTouchListener(new View.OnTouchListener() {
            private float initialX, initialY;
            private float initialTranslationX, initialTranslationY;
            private boolean isDragging;
            private final float touchSlop = ViewConfiguration.get(MainActivity.this).getScaledTouchSlop();
                    ...
                    case MotionEvent.ACTION_MOVE:
                        ...
                        if (isDragging) {
                            ...
                            if (parent != null) {
                                ...
                                newTranslationY = MathUtils.clamp(newTranslationY, minY, maxY);
                            }
    ...
    private void saveHomeOverlayPosition(View v) {
        ...
        prefs.get().edit()
                ...
                .putFloat("termux_pro_home_overlay_y_fraction", yFraction)
                .apply();
    }
    ...
    private void restoreHomeOverlayPosition(View v) {
        ...
        if (!prefs.get().contains("termux_pro_home_overlay_x_fraction")) return;

        float xFraction = prefs.get().getFloat("termux_pro_home_overlay_x_fraction", -1f);
        float yFraction = prefs.get().getFloat("termux_pro_home_overlay_y_fraction", -1f);
    ...
    private void openSessionCenter() {
        DesktopNavigationState.enterCliForeground();

        Intent intent = new Intent();
        intent.setClassName(getPackageName(), "com.termux.app.TermuxActivity");
        intent.putExtra("com.termux.app.extra.SHOW_SESSION_CENTER", true);
    ...
    void onReceiveConnection(Intent intent) {
        // Any incoming connection cancels the delayed foregrounding timer to prevent overlapping
        cancelPendingLaunch();

        Log.i("MainActivity", "ACTION_START_RECEIVED action=" + (intent == null ? null : intent.getAction()));
        Bundle bundle = extractConnectionBundle(intent);

        IBinder ibinder = extractConnectionBinder(bundle);
        if (ibinder == null && bundle == null) {
            // Try to recover from cached state if the Intent was launched without extras
            bundle = DesktopNavigationState.getLastConnectionBundle();
            ibinder = extractConnectionBinder(bundle);
        }

        if (ibinder == null) {
            Log.e("MainActivity", "CONNECTION_BINDER_PRESENT false; ACTION_START received without connection binder; bundlePresent=" + (bundle != null));
            DesktopNavigationState.setDesktopStartFailureReason("Missing connection binder in ACTION_START");
            
            // Update UI to show "Connecting..." instead of "Not connected" if we are in a boot state
            updateStubText("Connecting to X11...");

            // Do not falsely mark connected.
            handler.removeCallbacks(tryConnectRunnable);
            handler.postDelayed(tryConnectRunnable, 100);
            return;
        }
        Log.i("MainActivity", "CONNECTION_BINDER_PRESENT binder=" + ibinder);

        service = ICmdEntryInterface.Stub.asInterface(ibinder);
        try {
            service.asBinder().linkToDeath(() -> {
                service = null;
                Log.w("MainActivity", "BINDER_ALIVE false (binder died); resetting renderer connection");
                runOnUiThread(() -> {
                    getLorieView().connect(-1);
                    clientConnectedStateChanged();
                });
            }, 0);
        } catch (RemoteException e) {
            Log.e("MainActivity", "Failed to link to binder death", e);
        }

        boolean binderAlive = service != null && service.asBinder().isBinderAlive();
        Log.i("MainActivity", "BINDER_ALIVE=" + binderAlive);
        ...
        try {
            ...
            if (intent != null && intent != getIntent()) {
                setIntent(intent);
                getIntent().putExtra(CmdEntryPoint.EXTRA_CONNECTION_BUNDLE, bundle);
    ...
    boolean tryConnect() {
        handler.removeCallbacks(tryConnectRunnable);
        boolean alreadyConnected = getLorieView().connected();
        Log.i("MainActivity", "tryConnect: service=" + (service != null)
            + ", alreadyConnected=" + alreadyConnected);

        if (alreadyConnected) {
            DesktopNavigationState.setDesktopReady();
            runOnUiThread(this::clientConnectedStateChanged);
            return true;
        }

        if (service == null) {
            Log.w("MainActivity", "tryConnect service is null; requesting native connection retry");
            updateStubText("Searching for X11 bridge...");
            getLorieView().requestConnection();
            handler.postDelayed(tryConnectRunnable, 1000);
            return false;
        }

        try {
            if (!service.asBinder().isBinderAlive()) {
                Log.w("MainActivity", "BINDER_ALIVE false in tryConnect; resetting service and retrying");
                service = null;
                updateStubText("Bridge connection lost; retrying...");
                handler.postDelayed(tryConnectRunnable, 500);
                return false;
            }

            ParcelFileDescriptor fd = service.getXConnection();
            Log.i("MainActivity", "X_CONNECTION_FD_PRESENT=" + (fd != null));
            if (fd == null) {
                handler.postDelayed(tryConnectRunnable, 100);
                return false;
            }
            ...
            // Detach exactly once.
            int detachedFd = fd.detachFd();
            getLorieView().connect(detachedFd);

            boolean connectedNow = getLorieView().connected();
            if (connectedNow) {
                DesktopNavigationState.completeDesktopLaunch();
                finishStartupDraw();
                getLorieView().triggerCallback();
                runOnUiThread(() -> {
                    findViewById(R.id.stub).setVisibility(View.GONE);
                    getLorieView().setVisibility(View.VISIBLE);
                    clientConnectedStateChanged();
                });
                ...
                handler.removeCallbacks(tryConnectRunnable);
                handler.postDelayed(tryConnectRunnable, 200);
            }
            return true;
        } catch (Exception e) {
            Log.e("MainActivity", "Something went wrong while we were establishing connection", e);
            DesktopNavigationState.setDesktopStartFailureReason("Connection exception: " + e.getMessage());
            service = null;
            handler.postDelayed(tryConnectRunnable, 1000);
        }
        return false;
    ...
    private void updateStubText(String text) {
        runOnUiThread(() -> {
    ...
    void onPreferencesChangedCallback() {
        if (isFinishing() || isDestroyed() || prefs == null || mInputHandler == null)
            return;
        prefs.recheckStoringSecondaryDisplayPreferences();
    ...
    private void setTerminalToolbarView() {
        ...
        Log.i("MainActivity", "setTerminalToolbarView connected=" + connected + " showAdditionalKbd=" + prefs.showAdditionalKbd.get());

        // Requirement: Show extra-keys bar if enabled, even when connected, to allow keyboard toggling.
        boolean showNow = !isInPictureInPictureMode && prefs.showAdditionalKbd.get() && prefs.additionalKbdVisible.get();

        pager.setVisibility(showNow ? View.VISIBLE : View.GONE);
    ...
    void clientConnectedStateChanged() {
        runOnUiThread(()-> {
            ...
            findViewById(R.id.stub).setVisibility(connected?View.GONE:View.VISIBLE);
