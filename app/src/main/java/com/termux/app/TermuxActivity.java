...
public final class TermuxActivity extends AppCompatActivity implements ServiceConnection {
    ...
    /**
     * The Session Center controller.
     */
    SessionCenterController mSessionCenterController;
    ...
    public static final String EXTRA_SHOW_SESSION_CENTER = "com.termux.app.extra.SHOW_SESSION_CENTER";
    public static final String EXTRA_SHOW_SESSION_CENTER_WITHOUT_ANIMATION =
        "com.termux.app.extra.SHOW_SESSION_CENTER_WITHOUT_ANIMATION";

    private static final String EXTRA_OPEN_SESSION_DRAWER = "com.termux.app.extra.OPEN_SESSION_DRAWER";
    ...
    private final Runnable mDesktopShutdownRunnable = new Runnable() {
        @Override
        public void run() {
            if (!DesktopNavigationState.isDesktopOwnerStopping()) return;

            if (!DesktopSessionOrchestrator.isDesktopProcessRunning()) {
                Logger.logInfo(LOG_TAG, "X11 process gone; completing owner transition");
                DesktopNavigationState.completeDesktopOwnerShutdown(mWatchedOwnerHandle);
                termuxSessionListNotifyUpdated();
                return;
            }

            mDesktopShutdownHandler.postDelayed(this, 500);
        }
    };

    private final Runnable mDesktopShutdownTimeoutRunnable = () -> {
        if (DesktopNavigationState.isDesktopOwnerStopping()) {
            Logger.logError(LOG_TAG, "X11 shutdown timed out (20s). Keeping state locked.");
            // Do not enable new starts to prevent concurrent X11 instances.
            mDesktopShutdownHandler.removeCallbacks(mDesktopShutdownRunnable);
        }
    };

    private final Runnable mDesktopBootRunnable = new Runnable() {
        @Override
        public void run() {
            // No percentage progress. Just keep checking if readiness state changed.
            termuxSessionListNotifyUpdated();
            mDesktopBootHandler.postDelayed(this, 1000);
        }
    };

    private final Runnable mDesktopBootTimeoutRunnable = () -> {
        if (DesktopNavigationState.getDesktopBootState() == DesktopNavigationState.DesktopBootState.BOOTING) {
            String reason = "Timed out waiting for X11 bridge ready.";
            Logger.logError(LOG_TAG, "Desktop boot timed out (20s)");
            DesktopNavigationState.setDesktopStartFailureReason(reason);
            DesktopNavigationState.setDesktopFailed();
            stopDesktopBootProgress();
            termuxSessionListNotifyUpdated();
        }
    };
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        DesktopResourceManager.setContext(this);
        ...
        String activeOwner = mPreferences.getSharedPreferences().getString("termux_pro_active_desktop_owner_handle", null);
        if (activeOwner != null) {
            DesktopNavigationState.claimDesktopOwner(activeOwner);
        }
        ...
        mSessionCenterController = new SessionCenterController(this, findViewById(R.id.session_center_container));
        mSessionCenterController.setVisible(true, false);

        // One-shot 1500 ms startup interaction gate
        mStartupInteractionHandler.postDelayed(() -> {
            mIsStartupInteractionBlocked = false;
            termuxSessionListNotifyUpdated();
        }, 1500);
    ...
    protected void onNewIntent(Intent intent) {
        ...
        if (intent.getBooleanExtra(EXTRA_SHOW_SESSION_CENTER, false)) {
            setIntent(intent);
            if (mSessionCenterController != null) {
                boolean animate = !intent.getBooleanExtra(EXTRA_SHOW_SESSION_CENTER_WITHOUT_ANIMATION, false);
                mSessionCenterController.setVisible(true, animate);
                setHomeOverlayVisible(false);
            }
            return;
        }

        if (intent.getBooleanExtra(EXTRA_OPEN_SESSION_DRAWER, false)) {
            setIntent(intent);
            if (mSessionCenterController != null) mSessionCenterController.setVisible(false);
            getDrawer().openDrawer(Gravity.LEFT);
            return;
        }

        if (mSessionCenterController != null) mSessionCenterController.setVisible(true);
        setIntent(intent);
    }

    private boolean hasDesktopSession() {
        if (mTermuxService == null) return false;
        for (TermuxSession session : mTermuxService.getTermuxSessions()) {
            if (DesktopSessionOrchestrator.isDesktopSession(session)
            ...
            TermuxInstaller.setupBootstrapIfNeeded(TermuxActivity.this, () -> {
                ...
                try {
                    ...
                    if (launchFailsafe) {
                        ...
                        // Manual start only from Session Center.
                        if (mSessionCenterController != null) mSessionCenterController.setVisible(true);
                    }
    ...
    private void setTermuxSessionsListView() {
        ...
        if (mSessionCenterController != null)
            mSessionCenterController.setSessions(mTermuxService.getTermuxSessions());
    }
    ...
    private void initHomeOverlay() {
        ...
        homeOverlay.setOnTouchListener(new View.OnTouchListener() {
            ...
            private final float touchSlop = ViewConfiguration.get(TermuxActivity.this).getScaledTouchSlop();
            ...
            public boolean onTouch(View v, MotionEvent event) {
                switch (event.getAction()) {
                    ...
                    case MotionEvent.ACTION_UP:
                        ...
                        if (!wasDragging) {
                            if (mSessionCenterController != null) {
                                mSessionCenterController.setVisible(true);
    ...
    public void onBackPressed() {
        if (getDrawer().isDrawerOpen(Gravity.LEFT)) {
            getDrawer().closeDrawers();
        } else if (mSessionCenterController != null && findViewById(R.id.session_center_container).getVisibility() == View.GONE) {
            mSessionCenterController.setVisible(true);
            setHomeOverlayVisible(false);
        } else {
            finishActivityIfNotFinishing();
    ...
    public void startDesktopBootProgress() {
        stopDesktopBootProgress();
        DesktopNavigationState.startDesktopBoot();

        String handle = DesktopNavigationState.getActiveDesktopOwnerHandle();
        if (handle != null) {
            mPreferences.getSharedPreferences().edit()
                    .putString("termux_pro_active_desktop_owner_handle", handle)
                    .apply();
        }

        mDesktopBootHandler.postDelayed(mDesktopBootRunnable, 500);
        termuxSessionListNotifyUpdated();
    }

    public void stopDesktopBootProgress() {
        mDesktopBootHandler.removeCallbacks(mDesktopBootRunnable);
        mDesktopBootHandler.removeCallbacks(mDesktopBootTimeoutRunnable);
    }

    public void watchDesktopOwnerShutdown(String ownerHandle) {
        mWatchedOwnerHandle = ownerHandle;
        mDesktopShutdownHandler.removeCallbacks(mDesktopShutdownRunnable);
        mDesktopShutdownHandler.removeCallbacks(mDesktopShutdownTimeoutRunnable);
        mDesktopShutdownHandler.postDelayed(mDesktopShutdownRunnable, 500);
        mDesktopShutdownHandler.postDelayed(mDesktopShutdownTimeoutRunnable, 20000);
    }

    public boolean isSessionCenterVisible() {
        View container = findViewById(R.id.session_center_container);
    ...
    public void termuxSessionListNotifyUpdated() {
        runOnUiThread(() -> {
            ...
            if (mSessionCenterController != null && mTermuxService != null)
                mSessionCenterController.setSessions(mTermuxService.getTermuxSessions());
        });
    }
            ...
            if (mIsVisible) {
                ...
                switch (intent.getAction()) {
                    ...
                    case "com.termux.x11.CmdEntryPoint.ACTION_START":
                        ...
                        // TermuxActivity only stops the local progress indicator.
                        stopDesktopBootProgress();
                        boolean openDesktopWhenReady = DesktopNavigationState.isDesktopPending();
                        DesktopNavigationState.setDesktopReady(connectionBundle);
                        termuxSessionListNotifyUpdated();
    ...
    public boolean isStartupInteractionBlocked() {
        return mIsStartupInteractionBlocked;
    }
