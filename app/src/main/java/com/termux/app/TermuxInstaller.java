...
final class TermuxInstaller {
    ...
    private static final AtomicBoolean BOOTSTRAP_SETUP_IN_PROGRESS = new AtomicBoolean(false);

    /** Performs bootstrap setup if necessary. */
    static void setupBootstrapIfNeeded(final Activity activity, final Runnable whenDone) {
        if (BOOTSTRAP_SETUP_IN_PROGRESS.getAndSet(true)) {
            Logger.logInfo(LOG_TAG, "Bootstrap setup already in progress; ignoring duplicate trigger.");
            return;
        }
        ...
        if (FileUtils.directoryFileExists(TERMUX_PREFIX_DIR_PATH, true)) {
            if (TermuxFileUtils.isTermuxPrefixDirectoryEmpty()) {
                ...
                updateStandardWelcomeMessage();
                updateStandardShellPrompt();
                whenDone.run();
                return;
            }
        ...
        new Thread() {
            ...
            public void run() {
                try {
                    ...
                    BOOTSTRAP_SETUP_IN_PROGRESS.set(false);
    ...
    private static void updateStandardWelcomeMessage() {
        ...
        try {
            ...
            String message = "Welcome to TermuxPRO!\n\n"
                ...
                + "Developer of TermuxPRO: https://github.com/gulbalamesiyev\n\n"
    ...
    private static void updateStandardShellPrompt() {
        File bashrcFile = new File(TERMUX_PREFIX_DIR_PATH, "etc/bash.bashrc");
        String oldPrompt = "[[ \"$PS1\" == '\\s-\\v\\$ ' ]] && PS1='\\[\\e[0;32m\\]\\w\\[\\e[0m\\] \\[\\e[0;97m\\]\\$\\[\\e[0m\\] '";
        String newPrompt = "[[ \"$PS1\" == '\\s-\\v\\$ ' ]] && PS1='\\[\\e[1;34m\\]┌──(TermuxPRO)-[\\w]\\n\\[\\e[1;34m\\]└─\\[\\e[1;32m\\]\\$\\[\\e[0m\\] '";

        try {
            if (!bashrcFile.isFile()) return;
            byte[] bytes = new byte[(int) bashrcFile.length()];
            try (FileInputStream input = new FileInputStream(bashrcFile)) {
                int offset = 0;
                int read;
                while (offset < bytes.length
                    && (read = input.read(bytes, offset, bytes.length - offset)) != -1) {
                    offset += read;
                }
            ...
            if (!content.contains(oldPrompt) || content.contains("┌──(TermuxPRO)-")) return;

            writeUtf8File(bashrcFile, content.replace(oldPrompt, newPrompt));
            Logger.logInfo(LOG_TAG, "Updated the standard Termux shell prompt.");
        } catch (Exception e) {
            Logger.logError(LOG_TAG, "Unable to update the standard Termux shell prompt: " + e.getMessage());
        }
    }
